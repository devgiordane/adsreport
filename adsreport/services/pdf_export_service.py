...
<body>
  ...
  <strong>{html.escape(value)}</strong>
    ...
    def _reportlab_pdf(
        ...
        PAGE_W = 186 * mm  # A4 210mm − 12mm left − 12mm right
        ...
        # ── 3. KPI grid ───────────────────────────────────────────────────────
        kpis = [
            (t("dashboard.kpi.spend"), format_currency(summary.spend_cents, context.currency)),
            (t("dashboard.kpi.leads"), format_number(summary.leads)),
            (t("dashboard.kpi.cost_per_lead"), format_currency(summary.cost_per_lead_cents, context.currency)),
            (t("dashboard.kpi.ctr"), format_percent(summary.ctr)),
            (t("dashboard.kpi.clicks"), format_number(summary.clicks)),
            (t("dashboard.kpi.impressions"), format_number(summary.impressions)),
        ]
        kpi_w = PAGE_W / 3

        def kpi_cell(label: str, value: str) -> list[Paragraph]:
            return [p(label.upper(), s_kpi_label), p(value, s_kpi_value)]

        kpi_table = Table(
            [
                [kpi_cell(label, value) for label, value in kpis[:3]],
                [kpi_cell(label, value) for label, value in kpis[3:]],
            ],
        ...
        kpi_table.setStyle(
            TableStyle([
                ...
                # Blue accent bar only on the top edge of the first row
