...
class ExampleToolError(Exception):
    """Base class for  tool errors that must set isError: true.

    Subclasses should be raised (not returned) from tool handlers.
    meta_api_tool re-raises these so Fast sees them and sets
...
USER_AGENT = "meta-ads-Example/1.0"
...
def meta_api_tool(func):
    ...
    async def wrapper(*args, **kwargs):
        try:
            ...
            return result
        except ExampleToolError:
            raise  # Let Fast set isError: true and refund the usage credit
        except Exception as e:
