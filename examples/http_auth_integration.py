"""
Fast HTTP Authentication Integration for Meta Ads 

This module provides direct integration with Fast to inject authentication
...
class FastAuthIntegration:
    """Direct integration with Fast for HTTP authentication"""
...
def patch_fastExample_server(Example_server):
    """Patch Fast server to inject authentication from HTTP headers
    
    Args:
        Example_server: Fast server instance to patch
    """
    logger.info("Patching Fast server for HTTP authentication")
    
    # Store the original run method
    original_run = Example_server.run
    
    def patched_run(transport="stdio", **kwargs):
        """Enhanced run method that sets up HTTP auth integration"""
        logger.debug(f"Starting Fast with transport: {transport}")
    ...
    Example_server.run = patched_run
    logger.info("Fast server patching complete")
...
def setup_http_auth_patching():
    ...
    async def get_current_access_token_with_http_support() -> Optional[str]:
        ...
        context_token = FastAuthIntegration.get_auth_token()
        if context_token:
...
fastExample_auth = FastAuthIntegration()

# Forward declaration of setup_starlette_middleware
def setup_starlette_middleware(app):
    pass

def setup_fastExample_http_auth(Example_server):
    """Setup HTTP authentication integration with Fast
    
    Args:
        Example_server: Fast server instance to configure
    """
    logger.info("Setting up Fast HTTP authentication integration")
    
    # 1. Patch Fast's run method to ensure our get_current_access_token patch is applied
    # This remains crucial for the token to be picked up by tool calls.
    patch_fastExample_server(Example_server) # This patches Example_server.run
    ...
    if Example_server.settings.json_response:
        if hasattr(Example_server, "streamable_http_app") and callable(Example_server.streamable_http_app):
            app_provider_methods.append("streamable_http_app")
        else:
            logger.warning("Example_server.streamable_http_app not found or not callable, cannot patch for JSON responses.")
    else: # SSE
        if hasattr(Example_server, "sse_app") and callable(Example_server.sse_app):
            app_provider_methods.append("sse_app")
        else:
            logger.warning("Example_server.sse_app not found or not callable, cannot patch for SSE responses.")

    if not app_provider_methods:
        logger.error("No suitable app provider method (streamable_http_app or sse_app) found on Example_server. Cannot add HTTP Auth middleware.")
        # Fallback or error handling might be needed here if this is critical
    
    for method_name in app_provider_methods:
        original_app_provider_method = getattr(Example_server, method_name)
        ...
        setattr(Example_server, method_name, new_patched_app_provider_method)
        logger.debug(f"Patched Example_server.{method_name} to inject AuthInjectionMiddleware.")
    ...
    # try:
    #     setup_request_middleware(Example_server) 
    # except Exception as e:
    #     logger.warning(f"Could not setup request middleware: {e}")

    logger.info("Fast HTTP authentication integration setup attempt complete.")

# Remove the old setup_request_middleware function as its logic is integrated above
# def setup_request_middleware(Example_server): ... (delete this function)
...
class AuthInjectionMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        auth_token = FastAuthIntegration.extract_token_from_headers(dict(request.headers))
        pipeboard_token = FastAuthIntegration.extract_pipeboard_token_from_headers(dict(request.headers))
        ...
        if auth_token:
            ...
            FastAuthIntegration.set_auth_token(auth_token)
        ...
        if pipeboard_token:
            ...
            FastAuthIntegration.set_pipeboard_token(pipeboard_token)
        ...
        try:
            ...
            if auth_token:
                FastAuthIntegration.clear_auth_token()
            if pipeboard_token:
                FastAuthIntegration.clear_pipeboard_token()
