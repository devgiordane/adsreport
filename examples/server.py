""" server configuration for Meta Ads API."""

from Example.server.fastExample import Fast
import argparse
import os
import sys
import webbrowser
...
Example_server = Fast("meta-ads")

# Register resource URIs
Example_server.resource(uri="meta-ads://resources")(list_resources)
Example_server.resource(uri="meta-ads://images/{resource_id}")(get_resource)


class StreamableHTTPHandler:
    """Handles stateless Streamable HTTP requests for Meta Ads """
        ...
        return {
            ...
            'error': {
                ...
                'data': {
                    ...
                    'documentation': 'https://github.com/pipeboard-co/meta-ads-Example'
                }
            },
            'id': request_body.get('id')
        }
...
def main():
    ...
    logger.info("Meta Ads  server starting")
    logger.debug(f"Python version: {sys.version}")
    logger.debug(f"Args: {sys.argv}")
    
    # Initialize argument parser
    parser = argparse.ArgumentParser(
        description="Meta Ads  Server - Model Context Protocol server for Meta Ads API",
        epilog="For more information, see https://github.com/pipeboard-co/meta-ads-Example"
    )
    ...
    parser.add_argument("--transport", type=str, choices=["stdio", "streamable-http"], 
                       default="stdio", 
                       help="Transport method: 'stdio' for  clients (default), 'streamable-http' for HTTP API access")
    ...
    if args.version:
        from meta_ads_Example import __version__
        logger.info(f"Displaying version: {__version__}")
        print(f"Meta Ads  v{__version__}")
        return 0
    ...
    if args.transport == "streamable-http":
        ...
        print(f"Starting Meta Ads  server with Streamable HTTP transport")
        ...
        Example_server.settings.host = args.host
        Example_server.settings.port = args.port
        Example_server.settings.stateless_http = True
        Example_server.settings.json_response = not args.sse_response
        ...
        try:
            from .http_auth_integration import setup_fastExample_http_auth
            
            # Setup the Fast HTTP auth integration
            setup_fastExample_http_auth(Example_server)
            logger.info("Fast HTTP authentication integration setup successful")
            print("✅ Fast HTTP authentication integration enabled")
            print("   - Bearer tokens via Authorization: Bearer <token> header")
            print("   - Direct Meta tokens via X-META-ACCESS-TOKEN header")
            
        except Exception as e:
            logger.error(f"Failed to setup Fast HTTP authentication integration: {e}")
            print(f"⚠️  Fast HTTP authentication integration setup failed: {e}")
            print("   Server will still start but may not support header-based auth")
        
        # Log final server configuration
        logger.info(f"Fast server configured with:")
        logger.info(f"  - Host: {Example_server.settings.host}")
        logger.info(f"  - Port: {Example_server.settings.port}")
        logger.info(f"  - Stateless HTTP: {Example_server.settings.stateless_http}")
        logger.info(f"  - JSON Response: {Example_server.settings.json_response}")
        logger.info(f"  - Streamable HTTP Path: {Example_server.settings.streamable_http_path}")
        
        # Start the Fast server with Streamable HTTP transport
        try:
            logger.info("Starting Fast server with Streamable HTTP transport")
            print(f"✅ Server configured successfully")
            print(f"   URL: http://{args.host}:{args.port}{Example_server.settings.streamable_http_path}/")
            print(f"   Mode: {'Stateless' if Example_server.settings.stateless_http else 'Stateful'}")
            print(f"   Format: {'JSON' if Example_server.settings.json_response else 'SSE'}")
            Example_server.run(transport="streamable-http")
        except Exception as e:
        ...
        Example_server.run(transport='stdio') 