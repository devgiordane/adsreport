...
from .api import meta_api_tool, ExampleToolError
from . import auth
from .http_auth_integration import FastAuthIntegration

logger = logging.getLogger(__name__)


class RateLimitError(ExampleToolError):
    """Raised on 429 so Fast sets isError: true in the  response."""
    pass


class DuplicationError(ExampleToolError):
    """Raised on all non-success duplication responses so Fast sets isError: true."""
    pass
...
async def _forward_duplication_request(resource_type: str, resource_id: str, access_token: str, options: Dict[str, Any]) -> str:
    """
    Forward duplication request to the cloud-hosted  API using dual-header authentication.
    
    This implements the dual-header authentication pattern for  server callbacks:
    ...
    try:
        ...
        pipeboard_token = FastAuthIntegration.get_pipeboard_token()
        token_source = "contextvar" if pipeboard_token else None
        facebook_token = FastAuthIntegration.get_auth_token()

        # Fallback: proxy injects pb_token into arguments when ContextVar
        # is unreachable (Starlette BaseHTTPMiddleware -> Fast dispatch breaks it)
        ...
        if not pipeboard_token:
            raise DuplicationError(json.dumps({
                ...
                "details": {
                    ...
                    "received_headers": "Check that the  server is forwarding the X-Pipeboard-Token header"
                }
            }, indent=2))
        ...
        base_url = os.environ.get("PIPEBOARD_API_BASE_URL", "https://Example.pipeboard.co")
        ...
        headers = {
            ...
            "User-Agent": "meta-ads-Example/1.0"
        }
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            ...
            if response.status_code == 200:
                ...
                # Raise so Fast sets isError: true in  response,
        ...
        raise  # Let Fast handle these to set isError: true
